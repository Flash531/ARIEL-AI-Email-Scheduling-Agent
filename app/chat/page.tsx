...
export default function Home() {
  ...
  const runAgent = async (overrideText?: string) => {
    ...
    try {
      ...
      // Strip any internal debug suffix like "\n\n_Tools used: ..._" the API may append
      const assistantText = (data.text || "No response.")
        .replace(/\n\n_Tools used:[\s\S]*$/, "")
        .trim() || "Done — no actions needed.";
      ...
      // Persist to localStorage for the History page
      try {
        const entry = {
        ...
        <div style={S.navCenter}>
          {loading ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
              {([["/","Chat"],["/settings","Settings"]] as [string,string][]).map(([href, label]) => (
                <a key={href} href={href} style={{
                  padding: "5px 13px", borderRadius: 20, fontSize: 12.5, fontWeight: href === "/" ? 600 : 500,
                  color: href === "/" ? "var(--accent)" : "var(--text-secondary)",
                  background: href === "/" ? "var(--accent-dim)" : "transparent",
                  border: href === "/" ? "1px solid var(--accent-glow)" : "1px solid transparent",
